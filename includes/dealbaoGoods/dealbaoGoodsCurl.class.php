...
class dealbaoGoodsCurl
{
    ...
    function dealbaoGetGoodsList($keyword=[]){
        ...
        $data =   $this->Client->Goods->getGoodsListByCateGroup($keyword);
        if($data['code']==1000012 ){
            $this->dealbaoNewAccessToken();

            $this->dealbaoGetGoodsList($keyword);
...
//    }
    ...
    function dealbaoGetGoodsCategory($keyword=[]){


        $keyword['language_id'] = $this->language->getLanguageId();
        $data = $this->Client->Cate->getGoodsCategory($keyword);

        if($data['code']==1000012 ){
            $this->dealbaoNewAccessToken();

            $this->dealbaoGetGoodsCategory($keyword);
